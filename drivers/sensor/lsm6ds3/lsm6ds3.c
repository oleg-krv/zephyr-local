...
static const uint16_t lsm6ds3_odr_map[] = {0, 12, 26, 52, 104, 208, 416, 833,
					1660, 3330, 6660};
...
static const uint16_t lsm6ds3_gyro_fs_map[] = {250, 500, 1000, 2000, 125};
...
static inline int lsm6ds3_reboot(struct device *dev)
{
	...
	if (lsm6ds3_boot_set(data->ctx, 1) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_accel_set_fs_raw(struct device *dev, uint8_t fs)
{
	...
	if (lsm6ds3_xl_full_scale_set(data->ctx, fs) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_gyro_set_fs_raw(struct device *dev, uint8_t fs)
{
	...
	if (lsm6ds3_gy_full_scale_set(data->ctx, fs) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_gyro_set_odr_raw(struct device *dev, uint8_t odr)
{
	...
	if (lsm6ds3_gy_data_rate_set(data->ctx, odr) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_accel_range_set(struct device *dev, int32_t range)
{
	...
	data->acc_gain = (lsm6ds3_accel_fs_sens[fs] * GAIN_UNIT_XL);
	return 0;
}
#endif
...
static int lsm6ds3_gyro_range_set(struct device *dev, int32_t range)
{
	...
	data->gyro_gain = (lsm6ds3_gyro_fs_sens[fs] * GAIN_UNIT_G);
	return 0;
}
#endif
			   ...
			   const struct sensor_value *val)
{
	...
	case SENSOR_CHAN_HUMIDITY:
		return lsm6ds3_shub_config(dev, chan, attr, val);
...
static int lsm6ds3_sample_fetch_accel(struct device *dev)
{
	...
	if (lsm6ds3_acceleration_raw_get(data->ctx, buf.u8bit) < 0) {
...
static int lsm6ds3_sample_fetch_gyro(struct device *dev)
{
	...
	if (lsm6ds3_angular_rate_raw_get(data->ctx, buf.u8bit) < 0) {
		LOG_DBG("Failed to read sample");
		return -EIO;
	}
...
static int lsm6ds3_sample_fetch_temp(struct device *dev)
{
	...
	if (lsm6ds3_temperature_raw_get(data->ctx, buf.u8bit) < 0) {
		LOG_DBG("Failed to read sample");
		return -EIO;
	}
...
#if defined(CONFIG_LSM6DS3_SENSORHUB)
static int lsm6ds3_sample_fetch_shub(struct device *dev)
{
	if (lsm6ds3_shub_fetch_external_devs(dev) < 0) {
...
static inline void lsm6ds3_accel_convert(struct sensor_value *val, int raw_val,
...
static inline int lsm6ds3_accel_get_channel(enum sensor_channel chan,
					    struct sensor_value *val,
					    struct lsm6ds3_data *data,
					    uint32_t sensitivity)
{
	uint8_t i;
	...
	case SENSOR_CHAN_ACCEL_X:
		lsm6ds3_accel_convert(val, data->acc[0], sensitivity);
		break;
	case SENSOR_CHAN_ACCEL_Y:
		lsm6ds3_accel_convert(val, data->acc[1], sensitivity);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		lsm6ds3_accel_convert(val, data->acc[2], sensitivity);
		break;
	case SENSOR_CHAN_ACCEL_XYZ:
		for (i = 0; i < 3; i++) {
			lsm6ds3_accel_convert(val++, data->acc[i], sensitivity);
		}
		break;
	default:
		return -ENOTSUP;
...
static int lsm6ds3_accel_channel_get(enum sensor_channel chan,
				     struct sensor_value *val,
				     struct lsm6ds3_data *data)
{
	return lsm6ds3_accel_get_channel(chan, val, data, data->acc_gain);
}

static inline void lsm6ds3_gyro_convert(struct sensor_value *val, int raw_val,
					uint32_t sensitivity)
{
	int64_t dval;
...
static inline int lsm6ds3_gyro_get_channel(enum sensor_channel chan,
					   struct sensor_value *val,
					   struct lsm6ds3_data *data,
					   uint32_t sensitivity)
{
	...
	case SENSOR_CHAN_GYRO_X:
		lsm6ds3_gyro_convert(val, data->gyro[0], sensitivity);
		break;
	case SENSOR_CHAN_GYRO_Y:
		lsm6ds3_gyro_convert(val, data->gyro[1], sensitivity);
		break;
	case SENSOR_CHAN_GYRO_Z:
		lsm6ds3_gyro_convert(val, data->gyro[2], sensitivity);
		break;
	case SENSOR_CHAN_GYRO_XYZ:
		for (i = 0; i < 3; i++) {
			lsm6ds3_gyro_convert(val++, data->gyro[i], sensitivity);
		}
		break;
	default:
		return -ENOTSUP;
...
static int lsm6ds3_gyro_channel_get(enum sensor_channel chan,
				    struct sensor_value *val,
				    struct lsm6ds3_data *data)
{
	return lsm6ds3_gyro_get_channel(chan, val, data,
					LSM6DS3_DEFAULT_GYRO_SENSITIVITY);
}

#if defined(CONFIG_LSM6DS3_ENABLE_TEMP)
static void lsm6ds3_gyro_channel_get_temp(struct sensor_value *val,
					  struct lsm6ds3_data *data)
{
...
#if defined(CONFIG_LSM6DS3_SENSORHUB)
static inline void lsm6ds3_magn_convert(struct sensor_value *val, int raw_val,
...
static inline int lsm6ds3_magn_get_channel(enum sensor_channel chan,
					   struct sensor_value *val,
					   struct lsm6ds3_data *data)
{
	int16_t sample[3];
	...
	idx = lsm6ds3_shub_get_idx(SENSOR_CHAN_MAGN_XYZ);
	if (idx < 0) {
	...
	case SENSOR_CHAN_MAGN_X:
		lsm6ds3_magn_convert(val, sample[0], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_Y:
		lsm6ds3_magn_convert(val, sample[1], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_Z:
		lsm6ds3_magn_convert(val, sample[2], data->magn_gain);
		break;
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6ds3_magn_convert(val, sample[0], data->magn_gain);
		lsm6ds3_magn_convert(val + 1, sample[1], data->magn_gain);
		lsm6ds3_magn_convert(val + 2, sample[2], data->magn_gain);
		break;
	default:
		return -ENOTSUP;
	}
...
static inline void lsm6ds3_hum_convert(struct sensor_value *val,
				       struct lsm6ds3_data *data)
{
	float rh;
	int16_t raw_val;
	...
	idx = lsm6ds3_shub_get_idx(SENSOR_CHAN_HUMIDITY);
	if (idx < 0) {
...
static inline void lsm6ds3_press_convert(struct sensor_value *val,
					 struct lsm6ds3_data *data)
{
	int32_t raw_val;
	int idx;

	idx = lsm6ds3_shub_get_idx(SENSOR_CHAN_PRESS);
	if (idx < 0) {
		LOG_DBG("external press/temp not supported");
		return;
	}
...
static inline void lsm6ds3_temp_convert(struct sensor_value *val,
					struct lsm6ds3_data *data)
{
	int16_t raw_val;
	int idx;
			       ...
			       struct sensor_value *val)
{
	...
	case SENSOR_CHAN_MAGN_Z:
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6ds3_magn_get_channel(chan, val, data);
		break;

	case SENSOR_CHAN_HUMIDITY:
		lsm6ds3_hum_convert(val, data);
		break;

	case SENSOR_CHAN_PRESS:
		lsm6ds3_press_convert(val, data);
		break;

	case SENSOR_CHAN_AMBIENT_TEMP:
		lsm6ds3_temp_convert(val, data);
		break;
#endif
	default:
		return -ENOTSUP;
...
static const struct sensor_driver_api lsm6ds3_api_funcs = {
...
static int lsm6ds3_init_chip(struct device *dev)
{
	...
	if (lsm6ds3_device_id_get(lsm6ds3->ctx, &chip_id) < 0) {
	...
	/* reset device */
	if (lsm6ds3_reset_set(lsm6ds3->ctx, 1) < 0) {
		return -EIO;
	}

	k_busy_wait(100);

	if (lsm6ds3_accel_set_fs_raw(dev,
				     LSM6DS3_DEFAULT_ACCEL_FULLSCALE) < 0) {
		LOG_DBG("failed to set accelerometer full-scale");
		return -EIO;
	}
	lsm6ds3->acc_gain = LSM6DS3_DEFAULT_ACCEL_SENSITIVITY;

	lsm6ds3->accel_freq = lsm6ds3_odr_to_freq_val(CONFIG_LSM6DS3_ACCEL_ODR);
	...
	lsm6ds3->gyro_gain = LSM6DS3_DEFAULT_GYRO_SENSITIVITY;

	lsm6ds3->gyro_freq = lsm6ds3_odr_to_freq_val(CONFIG_LSM6DS3_GYRO_ODR);
	...
	if (lsm6ds3_fifo_mode_set(lsm6ds3->ctx, LSM6DS3_BYPASS_MODE) < 0) {
		LOG_DBG("failed to set FIFO mode");
		return -EIO;
	}

	if (lsm6ds3_block_data_update_set(lsm6ds3->ctx, 1) < 0) {
...
static const struct lsm6ds3_config lsm6ds3_config = {
	.bus_name = DT_INST_BUS_LABEL(0),
#if DT_ANY_INST_ON_BUS_STATUS_OKAY(spi)
	.bus_init = lsm6ds3_spi_init,
	...
	.spi_conf.cs        =  &lsm6ds3_data.cs_ctrl,
#else
	.spi_conf.cs        = NULL,
#endif
...
#if defined(CONFIG_LSM6DS3_INT_PIN_1)
	.int_pin = 1,
#elif defined(CONFIG_LSM6DS3_INT_PIN_2)
	.int_pin = 2,
#endif /* CONFIG_LSM6DS3_INT_PIN */
...
{
	const struct lsm6ds3_config * const config = dev->config_info;
	struct lsm6ds3_data *data = dev->driver_data;
