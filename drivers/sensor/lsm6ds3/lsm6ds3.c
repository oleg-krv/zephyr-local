...
static int lsm6ds3_accel_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (lsm6ds3_xl_full_scale_set(ctx, fs) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_gyro_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (lsm6ds3_gy_full_scale_set(ctx, fs) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_gyro_set_odr_raw(const struct device *dev, uint8_t odr)
{
	...
	if (lsm6ds3_gy_data_rate_set(ctx, odr) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_sample_fetch_accel(const struct device *dev)
{
	...
	if (lsm6ds3_acceleration_raw_get(ctx, buf.u8bit) < 0) {
...
static int lsm6ds3_sample_fetch_gyro(const struct device *dev)
{
	...
	if (lsm6ds3_angular_rate_raw_get(ctx, buf.u8bit) < 0) {
		LOG_DBG("Failed to read sample");
		return -EIO;
	}
...
static int lsm6ds3_sample_fetch_temp(const struct device *dev)
{
	...
	if (lsm6ds3_temperature_raw_get(ctx, buf.u8bit) < 0) {
		LOG_DBG("Failed to read sample");
		return -EIO;
	}
...
#if defined(CONFIG_LSM6DS3_SENSORHUB)
static int lsm6ds3_sample_fetch_shub(const struct device *dev)
{
					   ...
					   struct lsm6ds3_data *data)
{
	...
	idx = lsm6ds3_shub_get_idx(data->dev, SENSOR_CHAN_MAGN_XYZ);
	if (idx < 0) {
				       ...
				       struct lsm6ds3_data *data)
{
	...
	idx = lsm6ds3_shub_get_idx(data->dev, SENSOR_CHAN_HUMIDITY);
	if (idx < 0) {
					 ...
					 struct lsm6ds3_data *data)
{
	...
	idx = lsm6ds3_shub_get_idx(data->dev, SENSOR_CHAN_PRESS);
	if (idx < 0) {
		LOG_DBG("external press/temp not supported");
		return;
	}
...
static int lsm6ds3_init_chip(const struct device *dev)
{
	...
	if (lsm6ds3_device_id_get(ctx, &chip_id) < 0) {
	...
	if (lsm6ds3_fifo_mode_set(ctx, LSM6DS3_BYPASS_MODE) < 0) {
		LOG_DBG("failed to set FIFO mode");
		return -EIO;
	}

	if (lsm6ds3_block_data_update_set(ctx, 1) < 0) {
...
#define LSM6DS3_DEFINE(inst)						\
	...
	static const struct lsm6ds3_config lsm6ds3_config_##inst =	\
		COND_CODE_1(DT_INST_ON_BUS(inst, spi),			\
			(LSM6DS3_CONFIG_SPI(inst)),			\
			(LSM6DS3_CONFIG_I2C(inst)));			\
