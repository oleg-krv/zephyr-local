...
static uint8_t shub_ext[LSM6DS3_SHUB_MAX_NUM_SLVS];
...
static int lsm6ds3_lis2mdl_init(struct lsm6ds3_data *data, uint8_t i2c_addr)
{
	uint8_t mag_cfg[2];
	...
	lsm6ds3_shub_write_slave_reg(data, i2c_addr,
...
static int lsm6ds3_lis2mdl_odr_set(struct lsm6ds3_data *data,
				   uint8_t i2c_addr, uint16_t freq)
{
	...
	lsm6ds3_shub_enable(data, 1);
	return 0;
}

static int lsm6ds3_lis2mdl_conf(struct lsm6ds3_data *data, uint8_t i2c_addr,
				...
				const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_lis2mdl_odr_set(data, i2c_addr, val->val1);
	default:
...
static int lsmdso_hts221_read_conv_data(struct lsm6ds3_data *data,
					uint8_t i2c_addr)
{
	uint8_t buf[16], i;
	...
	for (i = 0; i < sizeof(buf); i += 7) {
		...
		if (lsm6ds3_shub_read_slave_reg(data, i2c_addr,
...
static int lsm6ds3_hts221_init(struct lsm6ds3_data *data, uint8_t i2c_addr)
{
	uint8_t hum_cfg;
...
static int lsm6ds3_hts221_odr_set(struct lsm6ds3_data *data,
				   uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
...
static int lsm6ds3_hts221_conf(struct lsm6ds3_data *data, uint8_t i2c_addr,
				enum sensor_channel chan,
				enum sensor_attribute attr,
				const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_hts221_odr_set(data, i2c_addr, val->val1);
	default:
...
static int lsm6ds3_lps22hb_init(struct lsm6ds3_data *data, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
static int lsm6ds3_lps22hh_init(struct lsm6ds3_data *data, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
static int lsm6ds3_lps22hh_odr_set(struct lsm6ds3_data *data,
				   uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
...
static int lsm6ds3_lps22hh_conf(struct lsm6ds3_data *data, uint8_t i2c_addr,
				enum sensor_channel chan,
				enum sensor_attribute attr,
				const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_lps22hh_odr_set(data, i2c_addr, val->val1);
	default:
...
static struct lsm6ds3_shub_slist {
	...
	uint8_t sh_out_reg;
	int (*dev_init)(struct lsm6ds3_data *data, uint8_t i2c_addr);
	int (*dev_conf)(struct lsm6ds3_data *data, uint8_t i2c_addr,
...
static inline void lsm6ds3_shub_wait_completed(struct lsm6ds3_data *data)
{
	uint16_t freq;
...
static inline void lsm6ds3_shub_embedded_en(struct lsm6ds3_data *data, bool on)
{
	if (on) {
		(void) lsm6ds3_mem_bank_set(data->ctx, LSM6DS3_SENSOR_HUB_BANK);
	} else {
		(void) lsm6ds3_mem_bank_set(data->ctx, LSM6DS3_USER_BANK);
	}

	k_busy_wait(150);
}

static int lsm6ds3_shub_read_embedded_regs(struct lsm6ds3_data *data,
					   uint8_t reg_addr,
					   uint8_t *value, int len)
{
	lsm6ds3_shub_embedded_en(data, true);

	if (lsm6ds3_read_reg(data->ctx, reg_addr, value, len) < 0) {
		LOG_DBG("shub: failed to read external reg: %02x", reg_addr);
		lsm6ds3_shub_embedded_en(data, false);
		return -EIO;
	}
...
static int lsm6ds3_shub_write_embedded_regs(struct lsm6ds3_data *data,
					    uint8_t reg_addr,
					    uint8_t *value, uint8_t len)
{
	lsm6ds3_shub_embedded_en(data, true);

	if (lsm6ds3_write_reg(data->ctx, reg_addr, value, len) < 0) {
...
static void lsm6ds3_shub_enable(struct lsm6ds3_data *data, uint8_t enable)
{
	...
	if (!data->accel_freq) {
		...
		if (lsm6ds3_xl_data_rate_set(data->ctx, odr) < 0) {
	...
	if (lsm6ds3_sh_master_set(data->ctx, enable) < 0) {
...
static int lsm6ds3_shub_check_slv0_nack(struct lsm6ds3_data *data)
{
	uint8_t status;
				       ...
				       uint8_t *value, uint16_t len)
{
	...
	if (lsm6ds3_shub_write_embedded_regs(data, LSM6DS3_SHUB_SLV0_ADDR,
					     slave, 3) < 0) {
	...
	/* turn SH on */
	lsm6ds3_shub_enable(data, 1);
	lsm6ds3_shub_wait_completed(data);

	/* read data from external slave */
	lsm6ds3_shub_embedded_en(data, true);
	if (lsm6ds3_read_reg(data->ctx, LSM6DS3_SHUB_DATA_OUT,
			     value, len) < 0) {
	...
	if (lsm6ds3_shub_check_slv0_nack(data) < 0) {
		lsm6ds3_shub_enable(data, 0);
		return -EIO;
	}
					...
					uint8_t *value, uint16_t len)
{
	...
	while (cnt < len) {
		...
		if (lsm6ds3_shub_write_embedded_regs(data,
 ...
 */
static int lsm6ds3_shub_set_data_channel(struct lsm6ds3_data *data)
{
	...
	if (lsm6ds3_sh_write_mode_set(data->ctx, wo) < 0) {
...
int lsm6ds3_shub_fetch_external_devs(struct device *dev)
{
	...
	for (n = 0; n < num_ext_dev; n++) {
		...
		if (lsm6ds3_read_reg(data->ctx, sp->sh_out_reg,
...
int lsm6ds3_shub_init(struct device *dev)
{
	...
	for (n = 0; n < ARRAY_SIZE(lsm6ds3_shub_slist); n++) {
		if (num_ext_dev >= LSM6DS3_SHUB_MAX_NUM_SLVS)
			break;
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (lsm6ds3_shub_read_slave_reg(data,
							sp->i2c_addr[i],
							sp->wai_addr,
							&chip_id, 1) < 0) {
	...
	lsm6ds3_shub_set_data_channel(data);
