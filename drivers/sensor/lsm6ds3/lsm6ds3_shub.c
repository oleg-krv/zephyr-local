...
#define LSM6DS3_SHUB_DATA_OUT				0x02

#define LSM6DS3_SHUB_SLV0_ADDR				0x15
#define LSM6DS3_SHUB_SLV0_SUBADDR			0x16
#define LSM6DS3_SHUB_SLV0_CONFIG			0x17
#define LSM6DS3_SHUB_SLV1_ADDR				0x18
#define LSM6DS3_SHUB_SLV1_SUBADDR			0x19
#define LSM6DS3_SHUB_SLV1_CONFIG			0x1A
#define LSM6DS3_SHUB_SLV2_ADDR				0x1B
#define LSM6DS3_SHUB_SLV2_SUBADDR			0x1C
#define LSM6DS3_SHUB_SLV2_CONFIG			0x1D
#define LSM6DS3_SHUB_SLV3_ADDR				0x1E
#define LSM6DS3_SHUB_SLV3_SUBADDR			0x1F
#define LSM6DS3_SHUB_SLV3_CONFIG			0x20
#define LSM6DS3_SHUB_SLV0_DATAWRITE			0x21

#define LSM6DS3_SHUB_STATUS_MASTER			0x22
#define LSM6DS3_SHUB_STATUS_SLV0_NACK		BIT(3)
#define LSM6DS3_SHUB_STATUS_ENDOP			BIT(0)

#define LSM6DS3_SHUB_SLVX_WRITE				0x0
#define LSM6DS3_SHUB_SLVX_READ				0x1

static int lsm6ds3_shub_write_slave_reg(struct lsm6ds3_data *data,
					uint8_t slv_addr, uint8_t slv_reg,
					uint8_t *value, uint16_t len);
static int lsm6ds3_shub_read_slave_reg(struct lsm6ds3_data *data,
				       uint8_t slv_addr, uint8_t slv_reg,
				       uint8_t *value, uint16_t len);
static void lsm6ds3_shub_enable(struct lsm6ds3_data *data, uint8_t enable);

/*
 * LIS2MDL magn device specific part
 */
#ifdef CONFIG_LSM6DS3_EXT_LIS2MDL
...
static int lsm6ds3_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6ds3_shub_write_slave_reg(dev, i2c_addr,
...
static int lsm6ds3_lis2mdl_odr_set(const struct device *dev,
				   uint8_t i2c_addr, uint16_t freq)
{
	...
	lsm6ds3_shub_enable(dev, 1);
	return 0;
}

static int lsm6ds3_lis2mdl_conf(const struct device *dev, uint8_t i2c_addr,
				...
				const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_lis2mdl_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DS3_EXT_LIS2MDL */

/*
 * HTS221 humidity device specific part
 */
#ifdef CONFIG_LSM6DS3_EXT_HTS221
...
static int lsmds3_hts221_read_conv_data(const struct device *dev,
					uint8_t i2c_addr)
{
	uint8_t buf[16], i;
	...
	for (i = 0; i < sizeof(buf); i += 7) {
		...
		if (lsm6ds3_shub_read_slave_reg(dev, i2c_addr,
...
static int lsm6ds3_hts221_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t hum_cfg;
	...
	return lsmds3_hts221_read_conv_data(dev, i2c_addr);
}

static const uint16_t hts221_map[] = {0, 1, 7, 12};

static int lsm6ds3_hts221_odr_set(const struct device *dev,
				   uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
...
static int lsm6ds3_hts221_conf(const struct device *dev, uint8_t i2c_addr,
				enum sensor_channel chan,
				enum sensor_attribute attr,
				const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_hts221_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DS3_EXT_HTS221 */

/*
 * LPS22HB baro/temp device specific part
 */
#ifdef CONFIG_LSM6DS3_EXT_LPS22HB
...
static int lsm6ds3_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
#endif /* CONFIG_LSM6DS3_EXT_LPS22HB */

/*
 * LPS22HH baro/temp device specific part
 */
#ifdef CONFIG_LSM6DS3_EXT_LPS22HH
...
static int lsm6ds3_lps22hh_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
static int lsm6ds3_lps22hh_odr_set(const struct device *dev,
				   uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
...
static int lsm6ds3_lps22hh_conf(const struct device *dev, uint8_t i2c_addr,
				enum sensor_channel chan,
				enum sensor_attribute attr,
				const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6ds3_lps22hh_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DS3_EXT_LPS22HH */

/* List of supported external sensors */
static struct lsm6ds3_shub_slist {
...
#ifdef CONFIG_LSM6DS3_EXT_LIS2MDL
	{
		...
		.out_data_addr  = 0x68,
		.out_data_len   = 0x06,
		.dev_init       = (lsm6ds3_lis2mdl_init),
		.dev_conf       = (lsm6ds3_lis2mdl_conf),
	},
#endif /* CONFIG_LSM6DS3_EXT_LIS2MDL */
...
#ifdef CONFIG_LSM6DS3_EXT_HTS221
	{
		...
		.out_data_len   = 0x02,
		.dev_init       = (lsm6ds3_hts221_init),
		.dev_conf       = (lsm6ds3_hts221_conf),
	},
#endif /* CONFIG_LSM6DS3_EXT_HTS221 */
...
#ifdef CONFIG_LSM6DS3_EXT_LPS22HH
	{
		...
		.out_data_addr  = 0x28,
		.out_data_len   = 0x05,
		.dev_init       = (lsm6ds3_lps22hh_init),
		.dev_conf       = (lsm6ds3_lps22hh_conf),
	},
#endif /* CONFIG_LSM6DS3_EXT_LPS22HH */
};

static inline void lsm6ds3_shub_wait_completed(stmdev_ctx_t *ctx)
{
	lsm6ds3_status_master_t status;

	do {
		k_msleep(1);
		lsm6ds3_sh_status_get(ctx, &status);
	} while (status.sens_hub_endop == 0);
}

static inline void lsm6ds3_shub_embedded_en(stmdev_ctx_t *ctx, bool on)
{
	if (on) {
		(void) lsm6ds3_mem_bank_set(ctx, LSM6DS3_SENSOR_HUB_BANK);
	} else {
		(void) lsm6ds3_mem_bank_set(ctx, LSM6DS3_USER_BANK);
	}

	k_busy_wait(150);
}

static int lsm6ds3_shub_read_embedded_regs(stmdev_ctx_t *ctx,
					   uint8_t reg_addr,
					   uint8_t *value, int len)
{
	lsm6ds3_shub_embedded_en(ctx, true);

	if (lsm6ds3_read_reg(ctx, reg_addr, value, len) < 0) {
		LOG_DBG("shub: failed to read external reg: %02x", reg_addr);
		lsm6ds3_shub_embedded_en(ctx, false);
		return -EIO;
	}
...
static int lsm6ds3_shub_write_embedded_regs(stmdev_ctx_t *ctx,
					    uint8_t reg_addr,
					    uint8_t *value, uint8_t len)
{
	lsm6ds3_shub_embedded_en(ctx, true);

	if (lsm6ds3_write_reg(ctx, reg_addr, value, len) < 0) {
...
static void lsm6ds3_shub_enable(const struct device *dev, uint8_t enable)
{
	...
	if (!data->accel_freq) {
		...
		if (lsm6ds3_xl_data_rate_set(ctx, odr) < 0) {
	...
	if (lsm6ds3_sh_master_set(ctx, enable) < 0) {
...
static int lsm6ds3_shub_check_slv0_nack(stmdev_ctx_t *ctx)
{
	uint8_t status;

	if (lsm6ds3_shub_read_embedded_regs(data, LSM6DS3_SHUB_STATUS_MASTER,
					     &status, 1) < 0) {
		LOG_DBG("shub: error reading embedded reg");
		return -EIO;
	}

	if (status & (LSM6DS3_SHUB_STATUS_SLV0_NACK)) {
 ...
 */
static int lsm6ds3_shub_read_slave_reg(const struct device *dev,
				       uint8_t slv_addr, uint8_t slv_reg,
				       uint8_t *value, uint16_t len)
{
	const struct lsm6ds3_config *cfg = dev->config;
	...
	slave[0] = (slv_addr << 1) | LSM6DS3_SHUB_SLVX_READ;
	slave[1] = slv_reg;
	slave[2] = (len & 0x7);

	if (lsm6ds3_shub_write_embedded_regs(ctx, LSM6DS3_SHUB_SLV0_ADDR,
					     slave, 3) < 0) {
	...
	/* turn SH on */
	lsm6ds3_shub_enable(dev, 1);
	lsm6ds3_shub_wait_completed(ctx);

	/* read data from external slave */
	lsm6ds3_shub_embedded_en(ctx, true);
	if (lsm6ds3_read_reg(ctx, LSM6DS3_SHUB_DATA_OUT,
			     value, len) < 0) {
	...
	if (lsm6ds3_shub_check_slv0_nack(ctx) < 0) {
		lsm6ds3_shub_enable(dev, 0);
		return -EIO;
	}
 ...
 */
static int lsm6ds3_shub_write_slave_reg(const struct device *dev,
					uint8_t slv_addr, uint8_t slv_reg,
					uint8_t *value, uint16_t len)
{
	...
	while (cnt < len) {
		slv_cfg[0] = (slv_addr << 1) & ~LSM6DS3_SHUB_SLVX_READ;
		slv_cfg[1] = slv_reg + cnt;

		if (lsm6ds3_shub_write_embedded_regs(ctx,
						     LSM6DS3_SHUB_SLV0_ADDR,
						     slv_cfg, 2) < 0) {
			LOG_DBG("shub: error writing embedded reg");
			return -EIO;
		}
 ...
 */
static int lsm6ds3_shub_set_data_channel(const struct device *dev)
{
	...
	for (n = 0; n < num_ext_dev; n++) {
		sp = &lsm6ds3_shub_slist[shub_ext[n]];

		i = n * 3;
		slv_cfg[i] = (sp->ext_i2c_addr << 1) | LSM6DS3_SHUB_SLVX_READ;
		slv_cfg[i + 1] = sp->out_data_addr;
		slv_cfg[i + 2] = sp->out_data_len;
	}

	if (lsm6ds3_shub_write_embedded_regs(ctx,
					     LSM6DS3_SHUB_SLV1_ADDR,
					     slv_cfg, n*3) < 0) {
		LOG_DBG("shub: error writing embedded reg");
		return -EIO;
	}
	...
	lsm6ds3_aux_sens_on_t aux = LSM6DS3_SLV_0_1_2;

	if (lsm6ds3_sh_slave_connected_set(data->ctx, aux) < 0) {
		LOG_DBG("shub: error setting aux sensors");
		return -EIO;
	}

	lsm6ds3_write_once_t wo = LSM6DS3_ONLY_FIRST_CYCLE;

	if (lsm6ds3_sh_write_mode_set(ctx, wo) < 0) {
...
int lsm6ds3_shub_get_idx(const struct device *dev, enum sensor_channel type)
{
	uint8_t n;
	struct lsm6ds3_data *data = dev->data;
	struct lsm6ds3_shub_slist *sp;
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		sp = &lsm6ds3_shub_slist[data->shub_ext[n]];
...
int lsm6ds3_shub_fetch_external_devs(const struct device *dev)
{
	...
	for (n = 0; n < num_ext_dev; n++) {
		...
		if (lsm6ds3_read_reg(ctx, sp->sh_out_reg,
...
{
	struct lsm6ds3_data *data = dev->data;
	struct lsm6ds3_shub_slist *sp = NULL;
	uint8_t n;
...
int lsm6ds3_shub_init(const struct device *dev)
{
	...
	for (n = 0; n < ARRAY_SIZE(lsm6ds3_shub_slist); n++) {
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (lsm6ds3_shub_read_slave_reg(dev,
							sp->i2c_addr[i],
							sp->wai_addr,
							&chip_id, 1) < 0) {
	...
	for (n = 0, regn = 0; n < data->num_ext_dev; n++) {
		sp = &lsm6ds3_shub_slist[data->shub_ext[n]];
		sp->sh_out_reg = LSM6DS3_SHUB_DATA_OUT + regn;
		regn += sp->out_data_len;
		sp->dev_init(dev, sp->ext_i2c_addr);
	}

	lsm6ds3_shub_set_data_channel(dev);
