...
//        struct k_delayed_work modem_creg_query_work;
...
//    modem_cmd_handler_set_error(data, 0);
    LOG_DBG("NO CARRIER");
    k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_no_carrier, K_MSEC(1));
    return 0;
}
...
//    int n, stat;
//    n = ATOI(argv[0], 0, "n");
//    stat = ATOI(argv[1], 0, "stat");
//    LOG_DBG("CREG: %d,%d", n, stat);
//    gsm.creg_status = stat;
//    k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_creg_change_status, K_MSEC(1));
////    k_sem_give(&gsm.sem_response);
//    return 0;
//}
...
//        MODEM_CMD("+CREG: ", gsm_cmd_creg, 2U, ","),
...
//        SETUP_CMD_NOHANDLE("AT+CGDCONT=1,\"IP\",\"" CONFIG_MODEM_GSM_APN "\""),
//        SETUP_CMD_NOHANDLE("AT+CGDCONT=1,\`"IP\",\"www.kyivstar.net\""),
//
//        SETUP_CMD_NOHANDLE("ATD*99***1#"),
//        SETUP_CMD_NOHANDLE("AT+CGATT?"),
...
static void net_connect(void) {
    ...
    if (ret < 0) {
        ...
        (void) k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_connect_network_query_work,
                                              K_SECONDS(5));
//        (void) k_delayed_work_submit_to_queue(&modem_work_q,&gsm.gsm_configure_work, K_SECONDS(1));
//            k_delayed_work_cancel(&modem_pins.workers.modem_creg_query_work);
//            (void) k_delayed_work_submit_to_queue(&modem_work_q, &gsm.gsm_configure_work, K_SECONDS(1));
    ...
    if (IS_ENABLED(CONFIG_GSM_MUX) && gsm->mux_enabled) {
        ...
        if (ret < 0) {
            LOG_DBG("modem setup returned %d, %s",
                    ret, "retrying...");
            (void) k_delayed_work_submit_to_queue(&modem_work_q, &gsm->gsm_configure_work,
                                                  K_SECONDS(1));
            return;
        }
    }
...
static void mux_setup_next(struct gsm_modem *gsm) {
    (void) k_delayed_work_submit_to_queue(&modem_work_q, &gsm->gsm_configure_work, K_MSEC(1));
}
...
static void mux_setup(struct k_work *work) {
    struct gsm_modem *gsm = CONTAINER_OF(work,
                                         struct gsm_modem,
                                         gsm_configure_work);
...
static void gsm_configure(struct k_work *work) {
    ...
    if (ret < 0) {
        ...
        (void) k_delayed_work_submit_to_queue(&modem_work_q, &gsm->gsm_configure_work, K_NO_WAIT);
        return;
    }
    ...
    if (IS_ENABLED(CONFIG_GSM_MUX) && ret == 0 &&
        ...
        LOG_DBG("GSM muxing %s", gsm->mux_enabled ? "enabled" : "disabled");

        if (gsm->mux_enabled) {
            gsm->state = STATE_INIT;

            k_delayed_work_init(&gsm->gsm_configure_work, mux_setup);
...
static void modem_power_on_work(struct k_work *work) {
    ...
    if (!modem_pins.power_ok) {
        modem_pins.power_ok = true;
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), power_gpios)
        do {
            k_sleep(K_MSEC(200));
            gpio_pin_set(modem_pins.pins.power, GSM_POWER_HW_PIN, 1);
            k_sleep(K_MSEC(3000));
            gpio_pin_set(modem_pins.pins.power, GSM_POWER_HW_PIN, 0);
            k_sleep(K_MSEC(3000));
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), status_gpios)
        } while (!modem_pins.input_pins.status);
#else
        } while (0);
#endif
    }
...
static void modem_reset_work(struct k_work *work) {
    modem_pins.power_ok = false;
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), reset_gpios)
    gpio_pin_set(modem_pins.pins.power, GSM_RESET_HW_PIN, 1);
    k_sleep(K_MSEC(3000));
    gpio_pin_set(modem_pins.pins.power, GSM_RESET_HW_PIN, 0);
#else
...
//    modem_disable_ppp();
    k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_connect_network_query_work,
                                   K_MSEC(1));
}
...
//	struct net_if *iface;
    ...
    if (work && gsm.operator_connected) {
        k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_rssi_query_work, GSM_RSSI_TIMEOUT);
    }
}
...
//    k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_creg_query_work, GSM_CREG_TIMEOUT);
}
...
static void modem_creg_change_status(struct k_work *work) {
    if (!(gsm.creg_status == 1 || gsm.creg_status == 5)) {
        if (gsm.operator_connected) {
            ...
            k_delayed_work_cancel(&modem_pins.workers.modem_rssi_query_work);
...
//            k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_rssi_query_work,
//                                           GSM_RSSI_TIMEOUT);
            k_delayed_work_submit_to_queue(&modem_work_q, &modem_pins.workers.modem_connect_network_query_work,
                                           K_MSEC(1));
        }
...
#endif
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), enable_gpios)
    gpio_pin_set(modem_pins.pins.enable, GSM_ENABLE_HW_PIN, 0);
#endif
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), sim_select_gpios)
    gpio_pin_set(modem_pins.pins.sim_select, GSM_SIM_SELECT_HW_PIN, 0);
#endif
#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), spk_enable_gpios)
    gpio_pin_set(modem_pins.pins.spk_enable, GSM_SPK_ENABLE_HW_PIN, 0);
#endif
}
...
static void interrupt_gsm_status(struct device *dev, struct gpio_callback *cb,
    ...
    ARG_UNUSED(cb);
    ARG_UNUSED(pins);
    modem_pins.input_pins.status = gpio_pin_get(modem_pins.pins.status,
                                                GSM_STATUS_HW_PIN);
    if (!modem_pins.input_pins.status && modem_pins.power_ok) {
        modem_disable_ppp();
        (void) k_delayed_work_submit_to_queue(&modem_work_q,
                                              &modem_pins.workers.modem_power_on_work,
                                              K_NO_WAIT);
    }
    if (modem_pins.input_pins.status && !modem_pins.power_ok) {
        (void) k_delayed_work_submit_to_queue(&modem_work_q,
                                              &modem_pins.workers.modem_power_off_work,
                                              K_NO_WAIT);
    }
}
...
static void modem_ppp_event_handler_ppp(struct net_mgmt_event_callback *cb,
    ...
    if ((mgmt_event & (NET_EVENT_PPP_CARRIER_ON | NET_EVENT_PPP_CARRIER_OFF | NET_EVENT_PPP_LINK_DEAD)) != mgmt_event) {
        return;
    }
...
static int gsm_init(struct device *device) {
    ...
    LOG_DBG("GSM modem SIMCOM (%p)", gsm);

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), power_gpios)
    modem_pins.pins.power = device_get_binding(GSM_POWER_HW_PORT);
    gpio_pin_configure(modem_pins.pins.power, GSM_POWER_HW_PIN, GPIO_OUTPUT);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), reset_gpios)
    modem_pins.pins.reset = device_get_binding(GSM_RESET_HW_PORT);
    gpio_pin_configure(modem_pins.pins.reset, GSM_RESET_HW_PIN, GPIO_OUTPUT);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), status_gpios)
    modem_pins.pins.status = device_get_binding(GSM_STATUS_HW_PORT);
    gpio_pin_configure(modem_pins.pins.status, GSM_STATUS_HW_PIN, GPIO_INPUT);
    gpio_init_callback(&modem_pins.gsm_status_cb, interrupt_gsm_status,
                       BIT(GSM_STATUS_HW_PIN));
    gpio_pin_interrupt_configure(modem_pins.pins.status, GSM_STATUS_HW_PIN,
                                 GPIO_INT_EDGE_BOTH);
    gpio_add_callback(modem_pins.pins.status, &modem_pins.gsm_status_cb);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), enable_gpios)
    modem_pins.pins.enable = device_get_binding(GSM_ENABLE_HW_PORT);
    gpio_pin_configure(modem_pins.pins.enable, GSM_ENABLE_HW_PIN, GPIO_OUTPUT);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), sim_select_gpios)
    modem_pins.pins.sim_select = device_get_binding(GSM_SIM_SELECT_HW_PORT);
    gpio_pin_configure(modem_pins.pins.sim_select, GSM_SIM_SELECT_HW_PIN, GPIO_OUTPUT);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), spk_enable_gpios)
    modem_pins.pins.spk_enable = device_get_binding(GSM_SPK_ENABLE_HW_PORT);
    gpio_pin_configure(modem_pins.pins.spk_enable, GSM_SPK_ENABLE_HW_PIN,
                       GPIO_OUTPUT);
#endif

#if DT_NODE_HAS_PROP(DT_INST(0, simcom_sim800), ring_gpios)
    modem_pins.pins.ring = device_get_binding(GSM_RING_HW_PORT);
    gpio_pin_configure(modem_pins.pins.ring, GSM_RING_HW_PIN, GPIO_INPUT);
#endif
    pin_init();
    ...
    k_thread_create(&gsm_rx_thread, gsm_rx_stack,
                    ...
                    gsm, NULL, NULL, K_PRIO_PREEMPT(2), 0, K_NO_WAIT);
    // K_PRIO_COOP(7)
    k_thread_name_set(&gsm_rx_thread, "gsm_rx");

    k_delayed_work_init(&gsm->gsm_configure_work, gsm_configure);
    k_delayed_work_init(&modem_pins.workers.modem_power_on_work, modem_power_on_work);
    k_delayed_work_init(&modem_pins.workers.modem_power_off_work, modem_power_off_work);
    k_delayed_work_init(&modem_pins.workers.modem_reset_work, modem_reset_work);
    k_delayed_work_init(&modem_pins.workers.modem_creg_change_status, modem_creg_change_status);
    k_delayed_work_init(&modem_pins.workers.modem_rssi_query_work, modem_rssi_query_work);
    k_delayed_work_init(&modem_pins.workers.modem_no_carrier, modem_no_carrier);

//    k_delayed_work_init(&modem_pins.workers.modem_creg_query_work, modem_creg_query_work);
    k_delayed_work_init(&modem_pins.workers.modem_connect_network_query_work, modem_connect_network_query_work);

    net_mgmt_init_event_callback(&modem_mgmt_cb_connect, modem_ppp_event_handler_connect,
                                 NET_EVENT_L4_CONNECTED | NET_EVENT_L4_DISCONNECTED);
    net_mgmt_init_event_callback(&modem_mgmt_cb_ppp, modem_ppp_event_handler_ppp,
                                 NET_EVENT_PPP_CARRIER_ON | NET_EVENT_PPP_CARRIER_OFF
...
void z_impl_modem_power_reset(void) {
    (void) k_delayed_work_submit_to_queue(&modem_work_q,
                                          &modem_pins.workers.modem_reset_work,
                                          K_NO_WAIT);
}
